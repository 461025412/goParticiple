...
// 热加载文件 保证文件被一个协程写完之后才会被另一个协程读取
var hotDictionaryFileToMemLock sync.RWMutex

var Segm = Segmenter{}

// 热加载
func HotLoadDictionary()  {
	for {
		MonitorDictionaryWait := sync.WaitGroup{}
		MonitorDictionaryWait.Add(1)
		// 起一个协程 保证正常能够解锁
		go func() {
			defer func() {
				if r := recover(); r != nil {
					runtimeErr := tool.PrintStack(r)
					logs.Error(fmt.Sprintf("MonitorDictionaryWait处理失败 panic: %v\n runtime error:%v", r, runtimeErr))
				}
				MonitorDictionaryWait.Done()
			}()

			// 判断文件是否存在
			if !tool.FileExists(bootstrap.ConfigData.WordPath) {
				time.Sleep(time.Second)
				return
			}

			updateTime := tool.GetFileModTime(bootstrap.ConfigData.WordPath)
			if updateTime > tool.DictionaryLoad.UpdateTime {
				//文件有变动 重新加载文件
				fmt.Println("开始加载Dictionary File到内存")
				segmenterTmp := Segmenter{}
				segmenterTmp.LoadDictionary(bootstrap.ConfigData.WordPath)

				hotDictionaryFileToMemLock.Lock()
				defer hotDictionaryFileToMemLock.Unlock()
				Segm.dict = segmenterTmp.dict

				tool.DictionaryLoad.UpdateTime = updateTime
			}

			return
		}()
		MonitorDictionaryWait.Wait()
		updateInterval := bootstrap.ConfigData.UpdateInterval
		updateIntervalDur := time.Duration(updateInterval) * time.Second
		time.Sleep(updateIntervalDur)
	}
}
